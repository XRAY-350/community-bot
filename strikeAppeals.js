...
async function _handleButton(interaction, state) {
  ...
  if (isApprove) {
    ...
    if (!member) return interaction.reply({ content: 'That member isn’t in the server anymore. I can’t safely change the strike/timeout from here. If they come back, use `/strike`.', flags: MessageFlags.Ephemeral });
    if (reduceTo !== null) {
...
// Thread ids where this member is the APPELLANT — corner's thread-strip must not eject them from their
// own strike appeal (audit U6, 2026-08-26).
function threadsFor(memberId) {
  const s = loadState();
  return Object.entries(s.appeals || {}).filter(([, a]) => a && a.memberId === memberId).map(([tid]) => tid);
}

module.exports = { setup, submit, handleButton, isConfigured, loadConfig, ensureBoard, threadsFor };
