...
// Fixed prefix (indices 0-12) never changes: header, age, div, mdni, div, region, div, language, div,
// notifications, div, pronouns, div. Misc occupies slot 13 ONLY when it has roles; when empty it (and its
// divider) are omitted so #roles doesn't show an empty "✨ Misc" heading, and Colors slides up to 13.
const SECTION_BLOCK_BASE = { age: 1, region: 5, language: 7, notifications: 9, pronouns: 11, misc: 13 };
function miscShown() { const s = loadSections(); return !!(s.misc && s.misc.length); }
// Block index of a section's heading in the CURRENT layout (accounts for a hidden, empty Misc). Misc maps
// to 13 whether shown or hidden — that slot start is where a misc add/remove must rebuild from either way.
function blockIndexOf(section) {
  if (section === 'colors') return miscShown() ? 15 : 13;
  return SECTION_BLOCK_BASE[section];
}
...
// call rebuildFromIndex(blockIndexOf(section)) afterward to actually push the change to Discord.
...
// Fixed prefix (slots 0-12) — index N always means the same thing there, so those sections' headings never
// move. Misc (slot 13) + Colors are the only variable tail: an empty Misc is omitted and Colors slides up
// from 15 to 13. blockIndexOf() above tracks this so rebuildFromIndex() always targets the right slot.
function buildBlocks(guild) {
  ...
  // Misc heading only appears when it holds roles (owner: don't show an empty Misc section).
  if (miscShown()) blocks.push(sectionBlock('misc'), dividerBlock());
  blocks.push(sectionBlock('colors'), dividerBlock(), tribeBlock(guild));
...
async function sweepDeadRoles(guild, channelId) {
  ...
  for (const key of Object.keys(removed)) await rebuildFromIndex(guild, channelId, blockIndexOf(key)).catch(() => {});
  return removed;
}
...
module.exports = {
  ...
  loadSections, addRoleToSection, removeRoleFromSection, SECTION_ORDER, SECTION_TITLE, blockIndexOf,
};
