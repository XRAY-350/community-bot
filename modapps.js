...
async function submitFromModal(interaction, config) { return withLock('modapps', () => _submitFromModal(interaction, config)); }
async function _submitFromModal(interaction, config) {
  const c = loadConfig();
...
// APPLICANT thread ids for this member's open applications — corner's thread-strip must not eject an
// applicant from their own application thread (audit U6, 2026-08-26). posts are keyed by REVIEW thread id;
// the applicant-facing thread is post.appThreadId.
function threadsFor(memberId) {
  const s = loadState();
  return Object.values(s.posts || {}).filter(p => p && p.applicantId === memberId && p.appThreadId).map(p => p.appThreadId);
}
...
module.exports = { setup, buildModal, positionRow, submitFromModal, handleButton, handlePositionSelect, handleAskModal, isConfigured, loadConfig, migrateLegacy, rerender, upgradeLegacyVotes, relayApplicantReply, backfillUndoButtons, sealOwnApplication, archiveOwnApplication, restoreArchived,
  ...
  getApprovers, addApprover, removeApprover, threadsFor };
