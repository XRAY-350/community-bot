...
function save(s) { try { atomicWriteJson(STATE_FILE, s, true); _cache = s; } catch (e) { _cache = s; console.error('[tribes] SAVE FAILED - state is IN MEMORY ONLY and will be lost on restart:', e.message); } }
...
// A leader holds the tribe's leader role — OR is listed in tribe.stateLeaderIds, a role-free leadership
// recognition (owner, 2026-08-23: a leader who wants the standing without wearing the role). Both paths
// count everywhere isLeader is used. (Server staff can also manage any tribe — callers add that.)
function isLeader(member, tribe) { return !!(member && tribe && ((tribe.leaderRoleId && member.roles.cache.has(tribe.leaderRoleId)) || (tribe.stateLeaderIds || []).includes(member.id))); }
// The tribe a member LEADS (holds the leader role of, or is a state-leader of), or null. A leader often
// isn't a rank-and-file member of their own tribe (holds the leader role, not the member role) — so "my
// tribe" checks both.
function leaderTribe(member) { if (!member) return null; return all().find(t => isLeader(member, t)) || null; }
...
// The rank index a member CURRENTLY holds, tracked internally (owner, 2026-08-24: rank rungs stopped
// being individual Discord roles — "keep the other markers internal" — this used to scan which rank ROLE
// a member held; now it reads tribe.memberRanks, the persisted record of the last rank they were actually
// promoted/set to). -1 if never ranked. Path mode: a stored index from a path the member has since
// switched AWAY from doesn't count (matches the old role-based behavior, where switching paths meant no
// longer holding any of the new path's role slots — see setMemberPath, which now clears the stored index
// on a switch instead of relying on role mismatch).
function currentRankIndex(member, tribe) {
  const id = typeof member === 'string' ? member : member.id;
  const ranks = tribe.ranks || [];
  const path = isPathMode(tribe) ? memberPath(tribe.key, id) : null;
  if (isPathMode(tribe) && !path) return -1;
  const idx = (tribe.memberRanks || {})[id];
  if (idx == null) return -1;
  if (path && ranks[idx] && ranks[idx].pathKey !== path) return -1;
  return idx;
}
// Persist the rank index a member was just set/promoted to — the write side of currentRankIndex above.
function setMemberRankIndex(key, userId, idx) {
  const s = load(); const t = s.tribes && s.tribes[key]; if (!t) return null;
  if (!t.memberRanks) t.memberRanks = {};
  if (idx == null || idx < 0) delete t.memberRanks[userId]; else t.memberRanks[userId] = idx;
  save(s); return idx;
}
...
// Switching paths starts the NEW path at rank 0 — no progress carries over (owner's call). Used to happen
// naturally (the member's old-path rank ROLE just didn't match the new path's 4 slots); now that rank is
// tracked internally rather than by role, the stored index has to be cleared explicitly here instead, or
// it would keep reading as the old path's rank number even though ranks[idx].pathKey no longer matches
// (currentRankIndex's own path-mismatch check catches this too, as a second layer — belt and suspenders).
function setMemberPath(key, userId, pathSlot) {
  ...
  if (t.memberRanks) delete t.memberRanks[userId];
...
module.exports = { load, save, all, get, getByRole, resolve, memberTribe, inAnyTribe, isMember, isLeader, leaderTribe, myTribe,
  ...
  addTides, getTides, topTides, recordJoin, tenureDays, earnedRankIndex, currentRankIndex, setMemberRankIndex, isPathMode,
