...
async function recomputeTier(guild, member, state, byTag) {
  ...
  // CORNERED member (audit A13): their strike role was stripped into the corner's snapshot, so live-role
  // edits here would leave them wearing TWO strike tiers after release (old one restored by uncorner, new
  // one added now). Mutate the corner snapshot instead — the right role comes back when they're released.
  const rec = state.getCornered(member.id);
  if (rec && Array.isArray(rec.roles)) {
    const before = rec.roles.length;
    rec.roles = rec.roles.filter(id => !ids.includes(id));
    if (target) rec.roles.push(target);
    state.setCornered(member.id, rec);
    console.log(`[strikes] ${member.id} is cornered — tier role swapped in the corner snapshot (${before}→${rec.roles.length} stored roles), not live`);
    return;
  }
  const held = ids.find(id => member.roles.cache.has(id)) || null;
  if (held === target) return;
  // Real error logging (audit U7): these were bare .catch(()=>{}) — the timed-out-member failure mode
  // below was completely invisible.
  if (held) await member.roles.remove(held, `strike tier recompute by ${byTag}`).catch(e => console.error(`[strikes] tier role remove (${member.id}):`, e.message));
  if (target) await member.roles.add(target, `strike tier recompute by ${byTag}`).catch(e => console.error(`[strikes] tier role add (${member.id}):`, e.message));
}
...
async function addStrike(guild, member, state, { weight, ruleIndex, reason, timeoutMs, byId, byTag }) {
  ...
  // Tier role BEFORE the timeout (audit U7): role edits FAIL on a timed-out member (the exact class
  // corner.js documents twice and works around with a lift/restore dance). The old order applied the
  // timeout first, then silently failed the tier-role swap — member's visible strike role stayed out of
  // sync with the ledger until the next boot resync.
