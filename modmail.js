...
function mailEmbed(num, text, revealedBy, senderId) {
  ...
  if (revealedBy) e.addFields({ name: 'Sender (revealed)', value: `<@${senderId}>, revealed by <@${revealedBy}>`, inline: true });
...
async function submit(guild, member, text) { return withLock('modmail', () => _submit(guild, member, text)); }
async function _submit(guild, member, text) {
  const c = loadConfig();
