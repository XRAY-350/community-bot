...
const config = require('./config');       // corner channel ids: corner system owns member overwrites there
...
// Atomic manifest write (audit U3): tmp + rename so a crash mid-write can't leave torn JSON that the
// loader would silently treat as "no baseline".
function saveManifest(man) {
  const tmp = MANIFEST_FILE + '.tmp';
  fs.writeFileSync(tmp, JSON.stringify(man, null, 2));
...
async function blessChannel(guild, channelId) {
  // Fetch FIRST, then load→mutate→write with zero awaits in between (audit U3): the old order held the
  // loaded manifest across two awaits, so any two concurrent blessers (MDNI lock × corner self-heal ×
  // the 2-min owner poll × syncModManageChannels' hourly loop) clobbered each other's entries — a channel
  // that WAS blessed silently reverted to its stale snapshot, and the next sweep "corrected" real
  // overwrites back to the old state. The likely root cause of every "the fix stuck, then came back".
  ...
  man[channelId] = entry;
  try { saveManifest(man); return true; } catch { return false; }
}

// Drop every overwrite for a deleted role from the golden manifest (audit U2). Called from the
// roleDelete listener; also used by the sweep when it discovers an unrestorable entry. Synchronous
// read-modify-write — safe against concurrent blessers.
function pruneRoleFromManifest(roleId) {
  const man = loadManifest(); if (!man) return 0;
  let pruned = 0;
  for (const entry of Object.values(man)) {
    const before = entry.overwrites.length;
    entry.overwrites = entry.overwrites.filter(o => !(o.type === 0 && o.id === roleId));
    pruned += before - entry.overwrites.length;
  }
  if (pruned) { try { saveManifest(man); } catch (e) { console.error('[permguard] prune save:', e.message); return 0; } }
  return pruned;
}
...
let _sweepBusy = false;   // re-entrancy guard (audit A21) — boot sweep vs first interval, or a slow sweep vs the next tick
async function sweepPermissions(guild, { notify = true } = {}) {
  ...
  if (_sweepBusy) return { fixed: 0, corrections: [], newMemberOverwrites: [], unmanagedChannels: 0, skipped: 'busy' };
  _sweepBusy = true;
  try {
  return await _sweepPermissionsInner(guild, manifest, { notify });
  } finally { _sweepBusy = false; }
}
async function _sweepPermissionsInner(guild, manifest, { notify = true } = {}) {
  const channels = [...(await guild.channels.fetch()).values()].filter(Boolean);
  const roles = await guild.roles.fetch();
  const corrections = [];
  const newMemberOverwrites = [];
  const selfGrants = [];            // member overwrites that GRANTED ViewChannel — reverted on sight
  const pruneQueue = [];            // deleted-role ids found in the manifest — pruned once, after the loop
  ...
  for (const ch of channels) {
    ...
    for (const roleId of allRoleIds) {
      ...
      // Role deleted from the guild entirely (audit U2): the golden entry is unrestorable — Discord
      // rejects every restore attempt (the 5 old language mini-mod roles retry-failed ~150×/48h each).
      // Prune it from the manifest instead of retrying forever — and if Discord left an ORPHANED live
      // overwrite for the dead role behind (it sometimes does — French Mini-Mod's survived on 3 channels),
      // delete that too so it can't linger as invisible clutter no UI can edit.
      if (!roles.get(roleId)) {
        if (live) await guild.client.rest.delete(Routes.channelPermission(ch.id, roleId)).catch(() => {});
        if (desired) pruneQueue.push(roleId);
        continue;
      }
    ...
    // Corner-system exemption, NARROWED (audit U1, 2026-08-26). The corner system manages its own dynamic
    // member overwrites (thread-jail lockouts: deny SendMessages / allow SendMessagesInThreads — see
    // corner.js applyThreadLockout). The SHARED corner is public-view (@everyone can see it), so member
    // overwrites there protect nothing — skip them all. The ADULT corner is a HIDDEN 18+ channel: only
    // corner's own NON-View-granting overwrites are exempt there — a member overwrite GRANTING ViewChannel
    // on it is a genuine self-grant and goes through the normal control below. (The first version of this
    // exemption skipped the whole adult channel too, silently disabling self-grant protection on a hidden
    // channel — caught by the 2026-08-26 audit.)
    const isSharedCorner = ch.id === config.cornerChannelId;
    const isAdultCorner = !!config.adultCornerChannelId && ch.id === config.adultCornerChannelId;
    for (const [memberId, live] of liveMember) {
      if (isSharedCorner) continue;
      const grantsView = live.allow.has(PermissionsBitField.Flags.ViewChannel);
      if (isAdultCorner && !grantsView) continue;
      // Golden member overwrites are VALUE-compared, not just presence-checked (audit A4): the old
      // `goldenMember.has() → continue` meant one blessed overwrite for member M was a permanent bypass —
      // M (or anyone with overwrite access) could later flip it to allow ViewChannel and every sweep
      // skipped it forever. A blessed entry only exempts the VALUES that were blessed; a View grant that
      // wasn't in the golden copy is a self-grant like any other.
      const golden = goldenMember.get(memberId);
      if (golden) {
        const goldenGrantsView = (BigInt(golden.allow || '0') & PermissionsBitField.Flags.ViewChannel) !== 0n;
        if (!grantsView || goldenGrantsView) continue;   // matches (or never granted View) — blessed, leave it
        // fall through: live grants View, golden didn't → treat as self-grant below
      }
      // The bot's OWN self-heal overwrite (healPermissions grants itself View+Send on the verify/conflict
      // channels) is not a self-grant (audit U4) — deleting it locked the bot out of #verify and posted a
      // 🚨 alert naming the bot itself.
      if (memberId === guild.client.user.id) continue;
      // A bot owner (incl. an owner alt) granting themselves channel access is legitimate — the self-grant
      // control exists to catch a MOD giving themselves visibility, not the owner. Leave owner overwrites
      // alone (e.g. the owner's alt's secret staff-chat access) and don't report or revert them.
      if (opspanel.isBotOwner({ id: memberId })) continue;
  ...
  // Prune unrestorable deleted-role entries in ONE synchronous pass after the (await-heavy) channel loop,
  // so we never hold a stale whole-manifest copy across awaits (audit U3's lost-update lesson).
  if (pruneQueue.length) {
    let pruned = 0;
    for (const roleId of new Set(pruneQueue)) pruned += pruneRoleFromManifest(roleId);
    if (pruned) console.log(`[permguard] pruned ${pruned} overwrite(s) for ${new Set(pruneQueue).size} deleted role(s) from the baseline`);
  }
...
async function isTrustedOwner(guild, userId) {
  ...
  if (opspanel.isBotOwner({ id: userId }) || userId === guild.ownerId) return true;
  const member = guild.members.cache.get(userId) || await guild.members.fetch(userId).catch(() => null);
  return !!member && opspanel.memberTier(member) === 'owner';
}

// Re-entrancy guard (audit A21): the 20-min sweep, the 2-min poll, and the two boot timers all reach this
// function; two overlapping runs read the same watermark and double-process/double-announce.
let _pollBusy = false;
async function pollOwnerOverwrites(guild) {
  if (!loadManifest()) return 0;   // no baseline yet — nothing to auto-adopt into
  if (_pollBusy) return 0;
  _pollBusy = true;
  try {
    ...
    // Empty audit page (audit A21): entries[-1].id threw a TypeError here, caught below, and the watermark
    // never advanced that tick. Keep the old watermark instead.
    if (entries.length) saveAuditState({ lastId: entries[entries.length - 1].id });
    return fresh.length;
...
module.exports = { sweepPermissions, resnapshot, loadManifest, blessChannel, pruneRoleFromManifest, register, computeDiff, openReconcile, isReconcileInteraction, handleReconcile, renderReconcile, isTrustedOwner, pollOwnerOverwrites };
