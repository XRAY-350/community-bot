...
// Thread ids where this member is a FRIEND on an open ban appeal — corner's thread-strip must not eject
// them from an appeal they opened or joined (audit U6, 2026-08-26).
function threadsFor(memberId) {
  const s = loadState();
  return Object.entries(s.appeals || {}).filter(([, a]) => a && (a.friends || []).includes(memberId)).map(([tid]) => tid);
}

module.exports = { setup, submit, handleButton, isConfigured, loadConfig, ensureBoard, reset, listDecided, threadsFor };
