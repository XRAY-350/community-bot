...
function systemRoleIds(config) {
  ...
  // Superlative / award roles (owner, 2026-08-23: "remove superlative roles from role request") — the
  // weekly peer-voted award category roles (e.g. "😂 Funniest Member") are EARNED by votes, not asked for.
  const awardRoles = Object.values(awards.categories()).map(c => c.roleId).filter(Boolean);
  ...
  return new Set([...STAFF, ...CHATTER, ...tribeRoles, ...awardRoles, config.modRoleId, config.verifiedRoleId, config.unverifiedRoleId,
