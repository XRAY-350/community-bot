...
// Computed PER CALL, not at module load (audit A25): the dashboard live-edits config.warnDays/kickDays,
// and a boot-frozen const kept enforcing the OLD grace window until restart while claiming the new one.
const graceMs = () => Math.max(0, config.kickDays - config.warnDays) * DAY;
...
function register(client, state, ctx) {
  ...
  // Self-rescheduling timeout instead of a fixed setInterval (audit A25): the dashboard live-edits
  // config.sweepIntervalMin and replied "applied", but the running interval was frozen at boot.
  const tick = () => { run(); setTimeout(tick, Math.max(1, config.sweepIntervalMin) * 60 * 1000).unref?.(); };
  setTimeout(tick, Math.max(1, config.sweepIntervalMin) * 60 * 1000);
...
async function sweep(client, state, ctx) {
  ...
  if (config.featureStale || config.assignUnverified) {
    for (const m of members.values()) {
      ...
      // Checked BOTH ways (audit A5): by role AND by corner state — a hand-removed corner role left the
      // state record set, this skip stopped matching, the backfill added Unverified, and the eventual
      // release restored Verified on top → dual-role conflict manufactured by the bot itself.
      ...
      if (state.getCornered && state.getCornered(m.id)) continue;
      ...
      if (config.dryRun) {
        ...
        warned += 1;
      } else if (now - st.warnedAt >= graceMs() && dueKick) {
