...
async function buildEventPanel(guild) {
  ...
  // Owner, 2026-08-22 re-scope: an Event Organizer's non-contest powers, spelled out so the panel is a
  // complete picture — they vote on new Organizer applications, and may /corner but ONLY in event channels
  // (not server-wide), and have no verify/ban/strike/watchlist.
  e.setFooter({ text: 'Also: vote on Event Organizer apps · /corner in event channels only · no verify/ban/strike.' });
...
module.exports = {
  CONTESTS, VOTE_EMOJI, CATEGORY_ID, isContestChannel, contestKeyForChannel,
