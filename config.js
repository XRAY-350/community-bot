...
const config = {
  ...
  // Application-gated staff-floor positions (owner, 2026-08-22), all built on staffapps.js — same staff
  // tier, NO cornering, empty default = inert until the role id is set. Media Team is the merged
  // Advertiser + Content Creator position (owner: "i was thinking advertisers would be creators") — it
  // both makes #showcase content (/create) and stages TikTok promos (/advertise). Live. Greeter + Support
  // Helper ship dark (feature off) until turned on.
  mediaRoleId: opt('MEDIA_ROLE_ID', ''),         // 🎬 Media Team — /create → #showcase, /advertise → TikTok staging
  greeterRoleId: opt('GREETER_ROLE_ID', ''),     // 👋 Greeter — welcomes + helps onboard newcomers (dark)
  supportRoleId: opt('SUPPORT_ROLE_ID', ''),     // 🛟 Support Helper — answers questions in a help space (dark)
  // Staff category (owner, 2026-08-22: "we can just add an advertisers chat to staff") — the coordination
  // channels + review forums for these positions are created under it so they inherit the staff-only
  // structure. publicCategoryId is where PUBLIC bot channels (e.g. #showcase) go — owner, 2026-08-22:
  // "stop creating channels with no category" (only the public-facing applicant channel may be category-less).
  staffCategoryId: opt('STAFF_CATEGORY_ID', '1516233713250471976'),   // 👤 Staff
  publicCategoryId: opt('PUBLIC_CATEGORY_ID', '1500215550020812850'), // ღ ᴄᴏᴍᴍᴜɴɪᴛʏ ఌ — home for public bot channels
  ...
  // Dedicated giveaways channel — the ONLY place /giveaway posts (owner, 2026-08-26: "remove the channel
  // option ... they should never be in the giveaway channel"). No setup() creates this (it predates one);
  // set explicitly if it's ever recreated.
  giveawayChannelId: opt('GIVEAWAY_CHANNEL_ID', '1541184500376142010'),
  ...
  editLogChannelId: opt('EDIT_LOG_CHANNEL_ID', '1541884191870816410'), // edited-message before/after — owner, 2026-08-25, mirrors deletion-log's shape/perms in its own channel
  ...
  // Adult + Active combined role (FUBU, owner 2026-08-24): bot-managed intersection of Adult Verified AND
  // an activity level (Intermediate Chatter+). Gates the 18+ selfies-and-flicks channel airtight — Discord
  // can't natively require two roles at once, so this single auto-granted/revoked role expresses the AND.
  // Empty on Melanin (no such channel/roles there) → the enforce/sweep functions no-op.
  adultActiveRoleId: opt('FUBU_ADULT_ACTIVE_ROLE_ID', ''),
  activeLevelRoleIds: opt('FUBU_ACTIVE_LEVEL_ROLE_IDS', '1529121181767176313,1529121191384842330,1529121471946035330').split(',').map(s => s.trim()).filter(Boolean),   // Intermediate / Elite / NOLIFE Chatter
  // "Active" reusable gate role (FUBU, owner 2026-08-24): bot-managed, granted to anyone holding any
  // activeLevelRoleId (Intermediate Chatter+). A single overwrite to gate a channel on activity level
  // (regular #selfies-n-flicks, plus future channels) instead of listing every level role per channel.
  // Empty on Melanin → enforce/sweep no-op.
  activeRoleId: opt('FUBU_ACTIVE_ROLE_ID', ''),
  ...
  // Threads the bot must NEVER auto-delete (owner, 2026-08-23) — e.g. a persistent verify thread kept as
  // a reference. deleteThread() (threads.js) skips any id in this list, so the verified→cleanup + orphan
  // reap leave it alone. Comma-separated thread ids, env-set per guild.
  protectedThreadIds: opt('PROTECTED_THREAD_IDS', '').split(',').map(s => s.trim()).filter(Boolean),
